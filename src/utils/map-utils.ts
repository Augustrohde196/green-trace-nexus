...
/**
 * Ensure assets are properly distributed on a map
 * @param assets Array of assets that need coordinates
 * @returns Assets with updated coordinates
 */
export function improveAssetDistribution(assets: any[]): any[] {
  // Create a copy to avoid mutating the original
  const updatedAssets = [...assets];
  
  // Only process if there are assets
  if (updatedAssets.length === 0) return updatedAssets;
  
  // Improve visual distribution of assets if coordinates are too close
  for (let i = 0; i < updatedAssets.length; i++) {
    const currentAsset = updatedAssets[i];
    
    if (!currentAsset.coordinates || !currentAsset.coordinates.lat || !currentAsset.coordinates.lng) {
      continue;
    }
    
    // Check if this asset is too close to any previous asset
    for (let j = 0; j < i; j++) {
      const previousAsset = updatedAssets[j];
      
      if (!previousAsset.coordinates || !previousAsset.coordinates.lat || !previousAsset.coordinates.lng) {
        continue;
      }
      
      const distance = calculateDistance(
        currentAsset.coordinates.lat, 
        currentAsset.coordinates.lng, 
        previousAsset.coordinates.lat, 
        previousAsset.coordinates.lng
      );
      
      // If assets are too close (less than 10km), slightly adjust the current one
      if (distance < 10) {
        // Add a small offset to make them visually distinct
        currentAsset.coordinates.lat += (Math.random() * 0.05 + 0.01) * (Math.random() > 0.5 ? 1 : -1);
        currentAsset.coordinates.lng += (Math.random() * 0.05 + 0.01) * (Math.random() > 0.5 ? 1 : -1);
      }
    }
  }
